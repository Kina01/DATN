...
class ClassService {
  static const String baseUrl = 'http://192.168.89.215:8080/api/classes';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ApiResponse.fromJson(
          data,
          (jsonData) =>
              (jsonData as List).map((e) => ClassSummary.fromJson(e)).toList(),
        );
      } else {
        return ApiResponse(
  ...
  static Future<ApiResponse<ClassResponse>> createClass(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ApiResponse.fromJson(
          data,
          (jsonData) => ClassResponse.fromJson(jsonData),
        );
      } else {
        return ApiResponse(
          message: 'Lỗi kết nối: ${response.statusCode}',
  ...
  static Future<ApiResponse<ClassResponse>> getClassDetail(int classId) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/getclass/$classId'),
        headers: {
          'Content-Type': 'application/json',
        },
      );
  ...
  static Future<ApiResponse<ClassResponse>> updateClass(
      int classId, UpdateClassRequest request) async {
    try {
      final user = await AuthService.getUser();
      if (user == null) {
        return ApiResponse(
      ...
      final response = await http.put(
        Uri.parse('$baseUrl/update/$classId'),
        headers: {
          'Content-Type': 'application/json',
          'User-ID': user.userId.toString(),
        },
